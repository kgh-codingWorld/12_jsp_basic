...
public class MyPage extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {		
		...
		if(id != null) {
			...
			//request.setAttribute("변수명", DAO에서 조회한 데이터);
			request.setAttribute("name"		, "김근휘");
			request.setAttribute("hp"		, "010-1234-5678");
			request.setAttribute("address"	, "경기");
			
			// 3-1-3) View로 페이지 이동 (데이터 포함)
			request.getRequestDispatcher("chapter07_session_cookie/07_01_session/myPage.jsp").forward(request, response);
