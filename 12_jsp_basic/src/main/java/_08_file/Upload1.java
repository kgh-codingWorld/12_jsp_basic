...
public class Upload1 extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (files.hasMoreElements()) { // 다음 데이터가 있으면(여러 번 사용할 경우는 while)
			
			String element = (String)files.nextElement();  // <input type="file"> 1개 반환
			...
			String contentType      = multipartRequest.getContentType(element); 	 // 업로드한 파일의 타입을 반환
